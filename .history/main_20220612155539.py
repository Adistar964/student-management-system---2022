...
def create_student():
    ...
    Button(r, text="click", command=lambda:print(data)).pack()
    def save_value(val, idx):
        data[idx] = val
    for idx,i in enumerate(labels[0:5]):
        row=0
        column=0
        if idx % 2 == 0:
            row = (idx)/2 + 1
        ...
        ent.config(validatecommand=lambda:save_value(ent.get(),idx),validate="focusout")

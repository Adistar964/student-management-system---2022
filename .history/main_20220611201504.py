...
def search_student(adm_no):
    ...
    r = Toplevel()
    r.resizable(0,0)
    Label(r, text="laaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", font=Font(size=20, underline=True, weight="bold")).pack(pady=14, padx=30)
    frame = Frame(r, highlightbackground="black", highlightthickness=2)
    frame.pack(ipady=10, ipadx=10,fill=X)
    for i in data.keys():
...
def main_window():
    ...
    update_btn = Button(text="update student Info", font=Font(size=15), bg="lightblue", fg="black")

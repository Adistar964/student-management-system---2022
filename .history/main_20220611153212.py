...
def search_student(adm_no):
    ...
    r = Toplevel()
    for i in data.keys():
        Label(r, text=i.replace("_", " ")+" : "+str(data[i])).pack()
    r.mainloop()
...
def main_window():
    ...
    search_btn = Button(command=lambda: operation_dialogue("update"),text="search student", font=Font(size=15), bg="lightblue", fg="black")

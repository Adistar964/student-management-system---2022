...
def create_student():
    ...
    Label(mainframe, text="Create New Student").grid(row=0,column=0,columnspan=2,pady=12)
    ...
    for idx,i in enumerate(labels_new):
        for jdx,j in enumerate(i):
            ...
            frame.grid(row=idx, column=jdx, ipadx=10, ipady=10)
        ...
        #     column=1
        # frame = Frame(mainframe, bg="white")
        # frame.grid(row=int(row), column=int(column), ipadx=10, ipady=10)
        # Label(frame, text=i.replace("_", " "), font=Font(size=15)).pack()
        # ent = Entry(frame,font=Font(size=10))
        # ent.pack(ipady=10, ipadx=70)
    r.mainloop()

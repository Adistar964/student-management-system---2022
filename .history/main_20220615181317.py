...
def create_student():
    r = Toplevel()
    r.state("zoomed")
    r.title("create new student")
    r.configure(bg="white")
    r.resizable(0,0)
    mainframe = Frame(r, bg="white")
    mainframe.pack(anchor="center")
    Label(mainframe, text="Create New Student",bg="white", font=Font(size=25, weight="bold", underline=True)).grid(row=0,column=0,columnspan=2,pady=20)
    cursor.execute("SELECT * FROM students")
    connection.commit()
    labels = list(dict(cursor.fetchone()).keys())
    labels.pop(0)
    labels_new = [labels[x:x+3] for x in range(0, len(labels), 3)]
    entries = []
    def print_all():
        new_entries = entries
        for idx,i in enumerate(entries):
            if len(i.get().strip()) == 0:
                new_entries.pop(idx)
            else:
                new_entries[idx] = i.get()
        if len(new_entries) != 17:
            messagebox.showerror("error!", "complete all fields!")
        else:
            cursor.execute("INSERT INTO students values(NULL,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",tuple(new_entries))
            connection.commit()
            r.destroy()
            search_student(cursor.fetchone())
            messagebox.showinfo("success!", "Successfully created the student!")
    for idx,i in enumerate(labels_new):
        for jdx,j in enumerate(i):
            frame = Frame(mainframe, bg="white")
            frame.grid(row=idx+1, column=jdx, ipadx=10, ipady=10)
            Label(frame, text=j.replace("_", " "), font=Font(size=15)).pack()
            ent = Entry(frame,font=Font(size=10))
            entries.append(ent)
            ent.pack(ipady=10, ipadx=70)
    Button(r, command=print_all,text="Create", font=Font(size=15), bg="darkgreen",fg="white").pack(anchor="center", pady=10, ipady=10, ipadx=20)
    r.mainloop()
...
def update_student(info):
    ...
    def update():
        try:
            ...
            cursor.execute(f"UPDATE students SET {value}='{input.get()}' WHERE admission_number={admission_number}")
            connection.commit()
...
def main_window():
    ...
    Label(w, text="Student Management System", font=Font(size=25), bg="CadetBlue", fg="black").pack(padx=100, pady=20)
    ...
    create_btn = Button(command=create_student, text="Create student", font=Font(size=15), bg="lightblue", fg="black")
...
def login_window():
    ...
    title = Label(r, text="Admin Login", bg="black", fg="white", font=Font(size=28, underline=True, family="bauhaus 93"))
    title.pack(pady=15)

    Label(r, text="", bg="black").pack(pady=10)

    user_label = Label(r, text="username", bg="black", fg="white", font=Font(size=15))
    user_label.pack(padx=16, pady=5) 

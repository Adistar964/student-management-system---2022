...
conn = sqlite3.connect("dict(student)s.db")
...
def update_student(student):
    ...
    f = Frame(r)
    ent = Entry(r, font=Font(size=10))
    ent.insert(student["name"])
    textVar = StringVar()
...
def main_window():
    ...
    Label(r, text="Student Operations:", font=Font(size=12)).pack(pady=10)
    Button(r, text="Create Student", font=Font(size=10), command=lambda:create_student()).pack(ipady=10,ipadx=17)
    ops = ["update","search","delete"]
    for i in ops:
        Button(r, text=i+" Student", font=Font(size=10), command=lambda i=i:op_win(i)).pack(ipady=10,ipadx=17)
    r.mainloop()

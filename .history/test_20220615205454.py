...
def create_student():
    ...
    Label(r, text="Create Student").pack()
    Label(r, text="").pack(pady=7)
    f = Frame(r)
    f.pack()
    ents = []
    ...
    for idx,i in enumerate(labels_3):
        for jdx,j in enumerate(i):
            fr = Frame(f)
            Label(fr, text=j).pack(pady=7)
            ent = Entry(fr, font=Font(size=10))
            ents.append(ent)
            ent.pack(ipady=10,ipadx=20)
...
def op_win(op):
    ...
    adm_no.pack(ipady=25,ipadx=10)
    Label(r, text="Your Password:", font=Font(size=12)).pack(pady=10)
    password = Entry(r, font=Font(size=10))
    password.pack(ipady=25,ipadx=10)
    Button(r, text="Confirm", bg="red", fg="white", command=do_op).pack(pady=12, ipady=15,ipadx=25)
    r.mainloop()
...
def main_window():
    ...
    r.geometry("700x700")
    r.resizable(0,0)
    Label(r, text="Student Management System:", font=Font(size=20, weight="bold")).pack(pady=15)
    Label(r, text="Student Operations:", font=Font(size=12, underline=True)).pack(pady=10)
    Button(r, bg="lightblue", fg="black",text="Create Student", font=Font(size=10), command=lambda:create_student()).pack(ipady=10,ipadx=17, pady=10)
    ops = ["Update","Search","Delete"]
    for i in ops:
        Button(r, bg="lightblue", fg="black",text=i+" Student", font=Font(size=10), command=lambda i=i:op_win(i)).pack(ipady=10,ipadx=17, pady=10)
    r.mainloop()

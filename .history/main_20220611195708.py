...
def search_student(adm_no):
    ...
    r = Toplevel()
    Label(r, text=data["name"], font=Font(size=20, underline=True, weight="bold")).pack(pady=14, padx=30)
    frame = Frame(r, highlightbackground="black", highlightthickness=2)
    frame.pack(ipady=10, ipadx=10,fill=X)
    for i in data.keys():
        row = Entry(frame, font=Font(size=15))
        row.insert(0,i.replace("_", " ")+" : "+str(data[i]))
        row["state"] = "disabled"
        row.pack(fill=X, padx=9, ipadx=7)
    r.mainloop()

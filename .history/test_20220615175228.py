...
def create_student():
    r = Toplevel()
    r.state("zoomed")
    Label(r, text="Create Student").grid(row=0,column=0,pady=10)
...
def main_window():
    ...
    Button(r, text="Create Student", command=create_student).pack(pady=10)
    ops = ["update","search","delete"]
    for i in ops:
        Button(r, text=i+" Student", command=lambda i=i:op_win(i)).pack(pady=10)
    r.mainloop()
